...
class UserHelper
{
    ...
    public function verifyErrorForRegisterWithForm($app, $user_override_vars, $expected_error_string)
    {
        ...
        foreach (array_keys($user_override_vars) as $key) {
            if ($user_override_vars[$key] === null) {
                unset($user_vars[$key]);
            }
        }

        // set password_confirmation
        if (!isset($user_vars['password_confirmation'])) {
            $user_vars['password_confirmation'] = $user_vars['password'];
        }
        ...
        if ($expected_error_string !== null) {
            throw new Exception("Did not find expected error: $expected_error_string", 1);
        }
    }
    ...
    {
        if ($data == true) {
            $response = $this->test_case->action('POST', 'Auth\AuthController@postBitcoinLogin', [], [
